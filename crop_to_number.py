...
DIGITS_DIR = r"/home/menashe/Downloads/image_proses/digits_font"        # התיקייה עם 0.png ... 9.png
IMAGES_DIR = r"/home/menashe/Downloads/image_proses/internal_session_20250930_1434"   # התיקייה עם כל הסקרינשוטים מהמשחק
DIGIT_SIZE = (40, 64)  # (width, height) – גודל אחיד לכל הספרות

DEBUG_DIR = Path("/home/menashe/Downloads/image_proses/debug_out")
...
def make_white_digit_mask(img_bgr: np.ndarray) -> np.ndarray:
    """
    מותאם לפינג לבן על רקע אפור (כמו בדוגמה 64ms)
    0   = ספרה
    255 = רקע
    """
...
def img_to_bw_digit(img: np.ndarray) -> np.ndarray:
    """
    ממיר תמונה (BGR/BGRA/GRAY/מסכה) למסכת ספרות סטנדרטית:
    0 = ספרה, 255 = רקע.
    מותאם לספרות לבנות/בהירות.
    """
    if img is None:
        raise ValueError("img_to_bw_digit got None")

    # אם יש אלפא – ניפטר ממנה
    ...
    if bgr is not None:
        ...
        if len(unique_vals) <= 4:
            ...
            # רוצים 0=ספרה, 255=רקע
            if np.mean(bw) > 127:
                bw = 255 - bw
        else:
            # תמונה רגילה → הפוך למסכה לבנה (0=ספרה)
...
def load_digit_templates(base_dir: str) -> Dict[int, List[np.ndarray]]:
    ...
    אם אצלך התיקיות 0..9 ישר תחת DIGITS_DIR, תחליף ל-base = Path(base_dir).
    """
...
def segment_bw_digits(bw_digits: np.ndarray,
    ...
    for label in range(1, num_labels):
        ...
        # לזרוק רק "גוש מסך" ענק (מצב img011)
        if (
        ...
        # --- חדש: להעיף אותיות קטנות כמו "ms" ---
        #if h < 0.50 * h_img:
        #    continue

        # --- חדש: פיצול קומפוננטה רחבה לשתי ספרות ---
        if w > 1.4 * h:  # אפשר לשחק עם היחס הזה
...
def crop_to_foreground(bw: np.ndarray, pad: int = 10) -> np.ndarray:
    """
    bw: 0=ספרה, 255=רקע
    מחזיר חיתוך סביב הפיקסלים של הספרות, עם padding.
    """
...
def split_wide_component(
    ...
    """
    מקבל קומפוננטה רחבה אחת (x,y,w,h) על המסכה bw_digits (0=ספרה,255=רקע),
    מנסה לפצל אותה לכמה קומפוננטות צרות יותר לפי פרופיל אנכי.
    אם לא נמצא פיצול טוב – מחזיר אותה כמו שהיא.
    """
...
def digit_similarity(bw_query: np.ndarray, bw_template: np.ndarray) -> float:
    """
    מחשב "מרחק" בין שתי מסכות – ממוצע |A-B|.
    כמה יותר נמוך → יותר דומה.
    """
...
def recognize_single_digit(
    ...
    """
    מזהה ספרה אחת לפי הטמפלטים.
    מחזיר:
        best_digit – הספרה הכי מתאימה
        scores     – מילון: ספרה -> שגיאה (נמוך יותר = טוב יותר)
    """
...
def test_recognize_number_from_bw_mask(
    ...
    # --- 1) סינון גיאומטרי: להעיף דברים שלא נראים כמו ספרות (כמו m) ---
    # מחשבים "גובה עיקרי" לפי הגובה המקסימלי
    ...
    for i, (digit_bw, bbox) in enumerate(segments):
        ...
        # קטן מדי יחסית לספרה הכי גבוהה -> רעש
        if h < 0.80 * max_h:
            continue

        # יחס רוחב/גובה: ספרות לרוב לא מאוד רחבות
        ar = w / float(h)
        if ar < 0.15 or ar > 0.85:
            # 'm' בדרך כלל רחב מדי, נקודות/רעשים קיצוניים גם יפלו
            continue
    ...
    # --- 2) לוקחים רק שתי ספרות שמאליות (כי ms מימין) ---
    filtered.sort(key=lambda t: t[1][0])  # sort by x
    #filtered = filtered[:2]  # ping בדרך כלל 2 ספרות (64)
...
PING_PATH = "/home/menashe/Downloads/image_proses/ping_region2.png"

def test_run(ping_path: str, templates):
    crop = cv2.imread(ping_path)
    if crop is None:
        raise FileNotFoundError(ping_path)
...
digits, number = test_run(PING_PATH, templates)
